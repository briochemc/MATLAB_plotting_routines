function [bandwidth,density,X,Y]=kde2d(data,n,MIN_XY,MAX_XY,dV)
...
%    
%           dV - A volume element weight that accompanies each data point
%                (Added by F. Primeau 10/28/2010)
...
initial_data=ndhist(transformed_data,n,dV);
...
%#######################################
function binned_data=ndhist(data,M,dV)
...
ikp = all(bins>0,2);
w = dV(ikp)/sum(dV(ikp));
binned_data = accumarray(bins(ikp,:),w,M(ones(1,ncols)));
end
